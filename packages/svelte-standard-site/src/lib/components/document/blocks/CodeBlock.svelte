<script lang="ts">
	...
	onMount(async () => {
		...
		try {
			...
			// shiki 4: bundledLanguages and bundledThemes are both Record<string, ...>
			const lang = block.language && block.language in bundledLanguages ? block.language : 'plaintext';
