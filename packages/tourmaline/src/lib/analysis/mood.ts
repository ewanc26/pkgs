...
export function buildMoodProfile(
  ...
  if (totalScrobbles > 0) {
    const nightHours =
      data.scrobblesByHour.slice(22, 24).reduce((a, b) => a + b, 0) +
      data.scrobblesByHour.slice(0, 5).reduce((a, b) => a + b, 0);
    const lateNightHours = data.scrobblesByHour
      .slice(0, 5)
      .reduce((a, b) => a + b, 0);
    const morningHours = data.scrobblesByHour
      .slice(6, 12)
      .reduce((a, b) => a + b, 0);
    ...
    const lateNightBoost = 1 + lateNightRatio * 0.1;
