/**
 * Browser-compatible Last.fm CSV parser.
 *
 * The parsing logic mirrors src/lib/csv.ts → parseLastFmCsvContent.
 * We avoid csv-parse (uses Node streams) and write a minimal parser instead.
 */
...
export function parseLastFmCsvContent(rawContent: string): LastFmCsvRecord[] {
  ...
  // Strip username comment from header
...
export function convertToPlayRecord(csv: LastFmCsvRecord): PlayRecord {
  const timestamp = parseInt(csv.uts);
