/**
 * Browser-based OAuth login flow for the Jasper CLI.
 *
  ...
  return new Promise((resolve) => {
    ...
    const server = http.createServer((req, res) => {
      ...
      res.writeHead(200, { "Content-Type": "text/html" }).end(`
        <!doctype html>
        <html>
          <head><meta charset="utf-8"><title>Jasper</title></head>
    ...
    timeoutId = setTimeout(() => {
      server.close(() =>
        resolve({
          success: false,
          error: "Timed out waiting for OAuth callback. Please try again.",
        }),
      );
    }, CALLBACK_TIMEOUT_MS);
  });
